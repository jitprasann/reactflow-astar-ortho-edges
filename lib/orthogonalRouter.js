...
export function computeOrthogonalPath(
  ...
  dist[startIdx] = 0;
  // When earlyBendBias > 0 (edge has a label): leave prevDir null so the
  // first step out of the stub costs nothing in either direction. The bias
  // then makes early horizontal turns cheaper, placing the horizontal segment
  // (and thus the label) close to the source node.
  // When earlyBendBias === 0 (no label): seed prevDir to the stub direction
  // so continuing straight costs nothing but the first turn costs bendPenalty,
  // preserving the original late-bend (compact) routing.
  prevDir[startIdx] = cfg.earlyBendBias > 0 ? null : (srcVertical ? 'v' : 'h');
  ...
  while (heap.size > 0) {
    ...
    for (const edge of adj[idx]) {
      ...
      // Small bias: horizontal segments further below the source stub end
      // cost slightly more, so among equal-bend paths the router picks the
      // one that goes horizontal earliest (closest to the source).
      let earlyBendCost = 0;
      if (srcVertical && edge.dir === 'h') {
        earlyBendCost = Math.max(0, waypoints[idx].y - sourceStubEnd.y) * cfg.earlyBendBias;
      }
    ...
    for (let i = 1; i < adjusted.length; i++) {
      ...
      if (Math.abs(prev.x - cur.x) > EPS && Math.abs(prev.y - cur.y) > EPS) {
        ...
        if (result.length === 2) {
          ...
          // Target stub junction — insert transition waypoint to bridge
          // the offset segment to the target stub orthogonally.
          // Works for both vertical stubs (top/bottom) and horizontal
          // stubs (left/right entry into merge nodes).
