...
function relaxEdge(edge, idx, cost, state, cfg, srcVertical, waypoints, sourceStubEnd, heap) {
...
function runDijkstra(adj, startIdx, endIdx, waypoints, cfg, srcVertical, sourceStubEnd) {
  ...
  while (heap.size > 0) {
    ...
    for (const edge of adj[idx]) {
      relaxEdge(edge, idx, cost, state, cfg, srcVertical, waypoints, sourceStubEnd, heap);
    }
  }
