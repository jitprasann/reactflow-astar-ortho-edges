 ...
 */
function inlineLastBranch(branchId, mergeId, pathNodes, edgeId, remainingEdge, edges, nodes, createEdge) {
    ...
    if (parentEdge && createEdge) {
        ...
        if (reconnect) {
            const assigned = assignHandles([normalizeEdge(reconnect)], newEdges);
            newEdges = newEdges.concat(assigned);
        }
    }
...
/**
 * Multiple branches remain after deletion — remove path nodes and reindex
 * handles on branch and merge nodes.
 */
function removePathAndReindex(branchId, mergeId, pathNodes, edgeId, edges, nodes) {
    ...
    if (mergeId) {
        ...
        newEdges = newEdges.map(function (e) {
            let updated = e;
