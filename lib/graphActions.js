...
function deleteBranchGroup(branchId, nodes, edges, createEdge) {
    var mergeId = findMergeNode(branchId, nodes, edges);

    // Collect all nodes between branch and merge
    var toRemove = new Set([branchId]);
    if (mergeId) toRemove.add(mergeId);

    var queue = edges.filter(function (e) { return e.source === branchId; }).map(function (e) { return e.target; });
    var visited = new Set();
    while (queue.length > 0) {
        ...
        toRemove.add(id);
        var children = edges.filter(function (e) { return e.source === id; }).map(function (e) { return e.target; });
        for (var c of children) queue.push(c);
    }

    // Find parent of branch and child of merge for reconnection
    var parentEdge = edges.find(function (e) { return e.target === branchId; });
    var mergeChildEdge = mergeId ? edges.find(function (e) { return e.source === mergeId; }) : null;

    var newNodes = nodes.filter(function (n) { return !toRemove.has(n.id); });
    var newEdges = edges.filter(function (e) { return !toRemove.has(e.source) && !toRemove.has(e.target); });

    // Reconnect parent → merge's child
    if (parentEdge && mergeChildEdge && createEdge) {
        var reconnect = createEdge(parentEdge.source, mergeChildEdge.target);
        if (reconnect) {
            var assigned = assignHandles([normalizeEdge(reconnect)], newEdges);
...
function deleteRegularNode(nodeId, nodes, edges, createEdge) {
    var inputEdges = edges.filter(function (e) { return e.target === nodeId; });
    var outputEdges = edges.filter(function (e) { return e.source === nodeId; });

    var newNodes = nodes.filter(function (n) { return n.id !== nodeId; });
    var newEdges = edges.filter(function (e) { return e.source !== nodeId && e.target !== nodeId; });

    // Simple chain: one input, one output → reconnect
    if (inputEdges.length === 1 && outputEdges.length === 1 && createEdge) {
        var parentEdge = inputEdges[0];
        var childEdge = outputEdges[0];
        var reconnect = createEdge(parentEdge.source, childEdge.target);
        if (reconnect) {
            var assigned = assignHandles([normalizeEdge(reconnect)], newEdges);
            newEdges = [].concat(newEdges, [assigned[0]]);
        }
...
function deleteBranchPath(edgeId, edge, nodes, edges, createEdge) {
    ...
    var remainingBranchEdges = edges.filter(function (e) { return e.source === branchId && e.id !== edgeId; });

    if (remainingBranchEdges.length === 1) {
        // Only one branch remains → inline it, remove branch + merge
        var remainingEdge = remainingBranchEdges[0];
        var remainingPathStartId = remainingEdge.target;

        var parentEdge = edges.find(function (e) { return e.target === branchId; });
        var mergeChildEdge = mergeId ? edges.find(function (e) { return e.source === mergeId; }) : null;

        // All nodes to remove: deleted path nodes + branch + merge
        var toRemove = new Set(pathNodes);
        toRemove.add(branchId);
        if (mergeId) toRemove.add(mergeId);

        var newNodes = nodes.filter(function (n) { return !toRemove.has(n.id); });
        var newEdges = edges.filter(function (e) {
        ...
        if (parentEdge && createEdge) {
            var reconnectParent = createEdge(parentEdge.source, remainingPathStartId);
            if (reconnectParent) {
                var assignedP = assignHandles([normalizeEdge(reconnectParent)], newEdges);
        ...
        if (mergeChildEdge && createEdge) {
            var pathEndEdge = edges.find(function (e) {
                return e.target === mergeId && !pathNodes.has(e.source) && e.source !== branchId;
            });
            var remainingPathEndId = pathEndEdge ? pathEndEdge.source : remainingPathStartId;

            var reconnectChild = createEdge(remainingPathEndId, mergeChildEdge.target);
            if (reconnectChild) {
                var assignedC = assignHandles([normalizeEdge(reconnectChild)], newEdges);
    ...
    var newNodes2 = nodes.filter(function (n) { return !pathNodes.has(n.id); });
    var newEdges2 = edges.filter(function (e) {
        return e.id !== edgeId && !pathNodes.has(e.source) && !pathNodes.has(e.target);
    });

    // Reindex source handles on branch node
    var srcEdges = newEdges2
        .filter(function (e) { return e.source === branchId; })
        .sort(function (a, b) {
            var ai = parseInt((a.sourceHandle || "output-0").split("-")[1], 10) || 0;
            var bi = parseInt((b.sourceHandle || "output-0").split("-")[1], 10) || 0;
            return ai - bi;
        });
    var srcRemap = new Map();
    srcEdges.forEach(function (e, i) { srcRemap.set(e.id, "output-" + i); });

    // Reindex target handles on merge node
    if (mergeId) {
        var tgtEdges = newEdges2
            .filter(function (e) { return e.target === mergeId; })
            .sort(function (a, b) {
                var ai = parseInt((a.targetHandle || "input-0").split("-")[1], 10) || 0;
                var bi = parseInt((b.targetHandle || "input-0").split("-")[1], 10) || 0;
                return ai - bi;
            });
        var tgtRemap = new Map();
...
export function cascadeDeleteNode(nodes, edges, nodeId, options) {
    var opts = options || {};
    var createEdge = opts.createEdge || null;
    ...
    if (node.data && node.data.isMerge) {
        var branchId = findOwningBranch(nodeId, nodes, edges);
        if (branchId) {
...
export function cascadeDeleteEdge(nodes, edges, edgeId, options) {
    var opts = options || {};
    var createEdge = opts.createEdge || null;
    var isBranchEdge = opts.isBranchEdge || null;
