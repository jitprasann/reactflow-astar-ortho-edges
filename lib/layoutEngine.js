...
function buildElkGraph(nodes, edges, cfg, interactive = false) {
    ...
    const children = nodes.map((node) => {
        ...
        targetHandles.forEach((handleId, i) => {
            ports.push({
                ...
                x: ((i + 1) / (targetHandles.length + 1)) * nodeWidth,
                y: 0,
                width: 1,
                height: 1,
                properties: {
        ...
        sourceHandles.forEach((handleId, i) => {
            ports.push({
                id: `${node.id}__${handleId}`,
                x: ((i + 1) / (sourceHandles.length + 1)) * nodeWidth,
                y: nodeHeight,
                width: 1,
                height: 1,
                properties: {
    ...
    if (interactive) {
        ...
        layoutOptions["org.eclipse.elk.layered.crossingMinimization.semiInteractive"] = "true";
    }
 ...
 * Preserves positions of existing nodes as much as possible:
 *   - Nodes at or above the parent keep both x and y.
 *   - Nodes below the parent keep x, but shift y to make room.
 *   - New nodes are positioned by ELK, then anchor-shifted to
 *     align with the parent's preserved position.
 *
 * @param {Array} existingNodes
 * @param {Array} existingEdges
 * @param {string} parentId - the parent node to attach new nodes below
...
export async function addNodesToLayout(
    ...
    // Run standard ELK layout to get correct layering for the new graph
    const elkGraph = buildElkGraph(allNodes, allEdges, cfg);
    const layouted = await elk.layout(elkGraph);
    const positioned = applyElkPositions(layouted, allNodes);

    const existingIds = new Set(existingNodes.map((n) => n.id));
    const originalPosMap = new Map(
        existingNodes.map((n) => [n.id, n.position]),
    );
    const elkPosMap = new Map(
        positioned.map((n) => [n.id, n.position]),
    );

    // Use parent node as anchor — compute how much ELK shifted it
    // so we can re-align new nodes beneath the parent's original position.
    const parentElk = elkPosMap.get(parentId);
    const parentOrig = originalPosMap.get(parentId);
    const anchorShiftX =
        parentElk && parentOrig ? parentOrig.x - parentElk.x : 0;
    const anchorShiftY =
        parentElk && parentOrig ? parentOrig.y - parentElk.y : 0;
    const parentOrigY = parentOrig ? parentOrig.y : 0;

    return positioned.map((n) => {
        if (existingIds.has(n.id)) {
            const orig = originalPosMap.get(n.id);
            if (orig && (orig.x !== 0 || orig.y !== 0)) {
                if (orig.y <= parentOrigY) {
                    // At or above parent level: preserve both x and y
                    // (siblings in the same layer stay put)
                    return { ...n, position: { x: orig.x, y: orig.y } };
                }
                // Below parent: keep original x, use anchored ELK y
                // (accounts for new layers inserted above)
                return {
                    ...n,
                    position: {
                        x: orig.x,
                        y: n.position.y + anchorShiftY,
                    },
                };
            }
        }
        // New node — use ELK position adjusted by anchor shift
        return {
            ...n,
            position: {
                x: n.position.x + anchorShiftX,
                y: n.position.y + anchorShiftY,
            },
        };
    });
