...
/**
 * Post-layout compaction: shrink vertical gaps between layers that have
 * no labeled edges between them from verticalGapWithLabel to verticalGap.
 */
function compactUnlabeledLayers(positionedNodes, edges, cfg) {
    ...
    if (bigGap <= smallGap) return positionedNodes;

    // Group nodes by approximate y-center into layers
    const nodeHeights = new Map(positionedNodes.map((n) => [
        n.id, { y: n.position.y, h: n.data?.height ?? cfg.nodeHeight },
    ]));
