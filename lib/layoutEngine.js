...
function compactUnlabeledLayers(positionedNodes, edges, cfg) {
    ...
    // Cluster nodes into ranks by y position (within a tolerance)
    const sorted = [...positionedNodes].sort((a, b) => a.position.y - b.position.y);
    const layers = [];
    let curLayer = null;
    for (const n of sorted) {
        const cy = n.position.y + ((n.data && n.data.height != null ? n.data.height : cfg.nodeHeight) / 2);
    ...
    for (let i = 1; i < layers.length; i++) {
        ...
        let hasLabel = false;
        for (const srcId of prevNodes) {
            for (const tgtId of curNodes) {
