...
function compactUnlabeledLayers(positionedNodes, edges, cfg) {
    ...
    for (const n of sorted) {
        const cy = n.position.y + (n.data?.height ?? cfg.nodeHeight) / 2;
    ...
    for (let i = 1; i < layers.length; i++) {
        ...
        if (!hasLabel) {
            for (const nId of curNodes) {
                ...
                if (nd?.data?.isMerge) { hasMerge = true; break; }
            }
        }

        const desiredGap = hasLabel ? maxGap : hasMerge ? mergeGap : normalGap;
